...
// "LULIB_GENERATE_IS_NESTING_TYPE(value_type)" は "is_nesting_value_type<T>" を生成する。
// "is_nesting_value_type<T>::value" は T が "T::value_type" を持っていると true となる。
// "LULIB_GENERATE_IS_NESTING_TYPE"マクロを使用した名前空間内にメタ関数を生成する
#define LULIB_GENERATE_IS_NESTING_TYPE(nesting_type)							\
	template<typename T>										\
