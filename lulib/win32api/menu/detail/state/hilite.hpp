...
namespace lulib { namespace win32api { namespace menu_detail { namespace state {
	...
	public:
		hilite(std::size_t id) : id_(id) {}

		template<HMENU (WINAPI *C)(), typename T>
		friend basic_menu<C,T>& operator<<(basic_menu<C,T>&, hilite&&);
	...
	basic_menu<T,Char>& operator<<(basic_menu<T,Char>& menu, hilite &&s) {
		// MenuItemInfo型
		typedef basic_menu_item_info<Char> menu_item_info;
		// MenuItemInfo
		menu_item_info mii;
		mii.state( item_type::state::hilite );
		// メニューアイテムを挿入
